...
- **Task 013-1: 핵심 기능 통합 테스트** - ✅ 완료 (F008/PDF는 Notion API 키 미준비로 제외, Task 013 완료 후 재검증 필요)
  ...
  - **검증 결과**: F001~F007, F009~F012 전 시나리오 PASS(버그 0건, `tsc`/`build` 무오류, 콘솔 에러 0건). F008(PDF)은 Task 013 보류로 제외

### Phase 4: 고급 기능 및 최적화

- **Task 014: 사용자 경험 향상 및 예외 처리 고도화** - ✅ 완료 (PDF 관련 항목은 Task 013 보류로 제외)
  ...
  - **구현 결과**: `lib/utils/toast.ts`(공용 토스트+세션만료 감지 유틸) 신규, 목록 삭제 낙관적 업데이트, 로딩 스켈레톤 실제 레이아웃 반영, 모바일 메뉴 Esc 닫힘 등 접근성 보강, offset 기반 페이지네이션(`components/reports-pagination.tsx`) 적용. 부수 발견 버그로 "전체 주차" 필터 선택 시 크래시(RSC 클라이언트 모듈 상수를 서버 컴포넌트에서 import한 문제)를 `lib/utils/week.ts`로 상수 이전해 수정. `profile-form.tsx` 토스트 통일과 무한 스크롤은 범위 밖으로 제외(간단한 페이지네이션으로 대체). Playwright 검증 및 `tsc`/`lint`/`build` 전부 통과
